...
class ImportFake:
    ...
    Class attributes:
        FakeSipModule: A fake for the sipconfig module.
        FakeSipConfig: A fake for the sipconfig.Configuration() object.

        Those are needed so sipconfig.Configuration().sip_version_str works.
    ...
    Attributes:
        ...
        sipconfig_exc: The exception sipconfig.Configuration() should raise,
                       or None.
        _real_import: Saving the real __import__ builtin so the imports can be
                      done normally for modules not in self.exists.
    """

    FakeSipConfig = collections.namedtuple('FakeSipConfig', 'sip_version_str')
    FakeSipModule = collections.namedtuple('FakeSipModule', 'Configuration')
    ...
    def __init__(self):
        ...
        self.version = '1.2.3'
        self.sipconfig_exc = None
        self._real_import = builtins.__import__

    def _sipconfig_configuration(self):
        """Fake for sipconfig.Configuration().

        Either provides sip_version_str with self.version, or raises
        self.sipconfig_exc if set.
        """
        if self.sipconfig_exc is not None:
            raise self.sipconfig_exc  # pylint: disable=raising-bad-type
        else:
            return self.FakeSipConfig(self.version)

    def _sipconfig_fake_import(self):
        """Provide a fake sip module. Called when sipconfig is imported."""
        if not self.exists['sipconfig']:
            raise ImportError("Fake ImportError for sipconfig.")
        else:
            return self.FakeSipModule(self._sipconfig_configuration)
    ...
    def _do_import(self, name):
        ...
        """
        if name == 'sipconfig':
            return self._sipconfig_fake_import()
        elif name not in self.exists:
...
class TestModuleVersions:
    ...
    def test_all_present(self):
        ...
        expected = ['SIP: 1.2.3', 'colorlog: yes', 'colorama: 1.2.3',
    ...
    @pytest.mark.parametrize('value, expected', [
        ('VERSION', ['SIP: 1.2.3', 'colorlog: yes', 'colorama: 1.2.3',
                     'pypeg2: yes', 'jinja2: yes', 'pygments: yes',
                     'yaml: yes']),
        (None, ['SIP: 1.2.3', 'colorlog: yes', 'colorama: yes', 'pypeg2: yes',
    ...
    def test_sipconfig_exception(self, exc, caplog, import_fake):
        """Test with sipconfig.Configuration() raising an exception.

        Args:
            exc: The exception to raise.
        """
        import_fake.sipconfig_exc = exc
        with caplog.atLevel(logging.ERROR, 'misc'):
            assert version._module_versions()[0] == 'SIP: ?'
        assert len(caplog.records()) == 1
        message = caplog.records()[0].message
        assert message == "Error while getting SIP version"

    def test_sipconfig_importerror(self, import_fake):
        """Test with sipconfig raising ImportError."""
        import_fake.exists['sipconfig'] = False
        assert version._module_versions()[0] == 'SIP: ?'
    ...
    def test_sip_version(self):
        """Make sure getting the SIP version works properly.

        The aim of this test is to fail if the sipconfig API changes, as the
        test above with the mocked sipconfig wouldn't detect that.
        """
        try:
            import sipconfig
        except ImportError as e:
            pytest.skip("Could not import sipconfig: {}".format(e))
        assert isinstance(sipconfig.Configuration().sip_version_str, str)
...
class FakeQSslSocket:
    ...
    Attributes:
        _supports: What QSslSocket::supportsSsl() should return.
        _version: What QSslSocket::sslLibraryVersionString() should return.
    """

    def __init__(self, supports, version=None):
...
class TestVersion:
    ...
    def test_ssl(self, monkeypatch):
        """Test SSL version in the output with SSL available."""
        monkeypatch.setattr('qutebrowser.utils.version.QSslSocket',
                            FakeQSslSocket(True, '1.0.1'))
        lines = version.version().splitlines()
        assert lines[7] == 'SSL: 1.0.1'

    def test_ssl_false(self, monkeypatch):
        """Test SSL version in the output with SSL unavailable."""
        monkeypatch.setattr('qutebrowser.utils.version.QSslSocket',
                            FakeQSslSocket(False))
        lines = version.version().splitlines()
        assert lines[7] == 'SSL: unavailable'

    def test_ssl_none(self, monkeypatch):
        """Test SSL version in the output with QSslSocket unavailable."""
        monkeypatch.setattr('qutebrowser.utils.version.QSslSocket', None)
        lines = version.version().splitlines()
        assert lines[7] == 'SSL: unavailable'
